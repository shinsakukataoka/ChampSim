...
# after the echo "[eval] TSV=..."
export CHAMPSIM_BIN="$ROOT/bin/champsim_L3_${MB}MB"

# change the parallel line to include --env CHAMPSIM_BIN (keeps it in the jobs)
parallel -j "$JOBS" --env CHAMPSIM_BIN --colsep '\t' --halt now,fail=1 \
